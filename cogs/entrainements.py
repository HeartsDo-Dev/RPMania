...
def reset_counter():
    ...
    conn.commit()
    logging.info("Il est l'heure de remettre les compteur à zéro ! (Remise à 0 des entrainements !)")
...
class Entrainements(commands.Cog):
    ...
    async def entrainement(self, ctx, row:str="None"):
        ...
        respond = respond[0]
        if respond == 2:
            return await ctx.send("Cooldown: Veillez patientez jusqu'a minuit pour vous entrainez de nouveau !", delete_after=20)
